...
select * from attachment where board_no = 62;
...
insert into authority values('kanggd', 'ROLE_USER');
insert into authority values('qwerty', 'ROLE_USER');
insert into authority values('admin', 'ROLE_USER');
insert into authority values('admin', 'ROLE_ADMIN');
insert into authority values('ohjang', 'ROLE_USER');
...
create table chat_member(
    ...
    created_at date default sysdate,
    deleted_at date,
    constraint pk_chat_member_id primary key(chatroom_id, member_id),
    constraint fk_chat_member_id foreign key(member_id) references member(member_id) -- 복합 pk
);
...
-- 관리자 채팅목록 조회
-- 채팅방 별로 최근 한건 조회해 목록화
select
    no,
    chatroom_id,
    (select member_id from chat_member where chatroom_id = cl.chatroom_id and member_id != 'admin') member_id, -- 채팅방에 관리자가 아닌 회원 아이디
    msg,
    time
from(
    select
...
insert into chat_log values(seq_chat_log_no.nextval, '6820m33E', 'admin', '네!', 1661760396620);